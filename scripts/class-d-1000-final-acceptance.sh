...
# historical typo families before syntax checks so no remote command depends on
# a later sed/parameter-substitution rescue.
...
for name, text in (('provision', p), ('campaign', c)):
    for forbidden in ('truy n', 'truin-d1000', 'truqyn', '/opt/truin'):
