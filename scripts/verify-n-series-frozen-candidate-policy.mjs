...
function fail(reason) {
  console.error(`TRUYN_N_SERIES_FROZEN_CANDIDATE_POLICY=FAIL reason=${reason}`);
  process.exit(1);
}
...
if (p.authority !== 'N-Series') fail('authority_changed');
if (p.effectiveUntil !== 'ALL_N_SERIES_TESTS_COMPLETE') fail('lifetime_changed');
...
const required = [
  ...
  'qualificationManifestMandatory',
...
for (const s of p.surfaces) {
  ...
  for (const b of s.blocks) if (!/^N[1-7]$/.test(b)) fail(`surface_${s.id}_block_invalid`);
  if (typeof s.liveRerunRequired !== 'boolean') fail(`surface_${s.id}_live_policy_missing`);
}
console.log('TRUYN_N_SERIES_FROZEN_CANDIDATE_POLICY=PASS model=frozen-candidate admission=mandatory main-movement=analysis-not-rerun');
