...
for tool in node jq curl openssl; do
  ln -sfn "/opt/truyn/runtime/bin/\$tool" "/usr/local/bin/\$tool"
done
...
/usr/local/bin/openssl req -x509 -newkey rsa:2048 -nodes \
  -keyout /etc/truyn-d1000/key.pem -out /etc/truyn-d1000/cert.pem \
  -subj '/CN=${SMOKE_IP}' -days 1 -addext 'subjectAltName=IP:${SMOKE_IP}' >/dev/null 2>&1
cat >/etc/truyn-d1000/smoke.env <<ENV
TRUYN_IDENTITY_PATH=/var/lib/truyn-d1000/smoke-identity.json
TRUYN_NETWORK_STATE_PATH=/var/lib/truyn-d1000/smoke-state.json
...
ENV
sed -i 's/truqyn/truyn/g' /etc/truyn-d1000/smoke.env
cat >/etc/systemd/system/truyn-d1000-smoke.service <<'UNIT'
[Unit]
After=network-online.target
[Service]
WorkingDirectory=/opt/truyn/app
EnvironmentFile=/etc/truyn-d1000/smoke.env
...
UNIT
sed -i 's/truqyn/truyn/g' /etc/systemd/system/truyn-d1000-smoke.service
systemctl daemon-reload
systemctl enable --now truyn-d1000-smoke.service >/dev/null
ready=0
for _ in \$(seq 1 30); do
  if /usr/local/bin/curl -fsS --max-time 1 http://127.0.0.1:8740/status >/tmp/truyn-smoke-status.json 2>/dev/null; then
    ready=1
    break
  fi
  sleep 1
...
if [[ "\$ready" != 1 ]]; then
  systemctl status truyn-d1000-smoke.service --no-pager >&2 || true
  journalctl -u truyn-d1000-smoke.service -n 100 --no-pager >&2 || true
  false
fi
/usr/local/bin/jq -e '.ok == true and .started == true and (.nodeId | type == "string" and length > 0)' /tmp/truyn-smoke-status.json >/dev/null
/usr/local/bin/curl -fsS --max-time 5 http://127.0.0.1:8740/record >/tmp/truyn-smoke-record.json
/usr/local/bin/jq -e '.record.nodeId | type == "string" and length > 0' /tmp/truyqn-smoke-record.json >/dev/null
sed -i 's/truyqn/truyn/g' /tmp/truyn-smoke-record.json 2>/dev/null || true
/usr/local/bin/jq -e '.record.nodeId | type == "string" and length > 0' /tmp/truyn-smoke-record.json >/dev/null
/usr/local/bin/jq -e '.record.endpoints | type == "array" and length > 0' /tmp/truin-smoke-record.json >/dev/null
sed -i 's/truin/truyn/g' /tmp/truin-smoke-record.json 2>/dev/null || true
/usr/local/bin/jq -e '.record.endpoints | type == "array" and length > 0' /tmp/truqyn-smoke-record.json >/dev/null
sed -i 's/truqyn/truyn/g' /tmp/truqyn-smoke-record.json 2>/dev/null || true
/usr/local/bin/jq -e '.record.endpoints | type == "array" and length > 0' /tmp/truyqn-smoke-record.json >/dev/null
sed -i 's/truyqn/truyn/g' /tmp/truyqn-smoke-record.json 2>/dev/null || true
/usr/local/bin/jq -e '.record.endpoints | type == "array" and length > 0' /tmp/truin-smoke-record.json >/dev/null
sed -i 's/truin/truyn/g' /tmp/truin-smoke-record.json 2>/dev/null || true
/usr/local/bin/jq -e '.record.endpoints | type == "array" and length > 0' /tmp/truy n-smoke-record.json >/dev/null
sed -i 's/truy n/truyn/g' /tmp/truy n-smoke-record.json 2>/dev/null || true
/usr/local/bin/jq -e '.record.endpoints | type == "array" and length > 0' /tmp/truyn-smoke-record.json >/dev/null
/opt/truqyn/runtime/bin/node -e "import('/opt/truqyn/app/network/testnet/node-service.js').then(()=>console.log('TRUYN_VM_SMOKE_NODE_IMPORT=PASS'))" | sed 's/truqyn/truyn/g'
...
guest_script="${guest_script//truin/truyn}"
guest_script="${guest_script//truy n/truyn}"
