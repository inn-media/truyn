...
function errorCode(error) {
  return error?.code || error?.name || error?.message || 'probe_lifecycle_error';
}
...
if (!AdversarialScaleNode.prototype.__truynScaleProbeTransportV3) {
  ...
  AdversarialScaleNode.prototype.probe = async function probeWithResolvedTransport(target, value, options = {}) {
    ...
    try {
      ...
      // A lifecycle or AbortSignal timeout is evidence of a failed transport
      // attempt, not a reason to abort the entire benchmark process. Normalizing it
      // here keeps the sample in the denominator. It can only make a routing or
      // integrity gate harder to pass; cryptographic failures are never promoted.
      return {
        ok: false,
        transportError: errorCode(error),
        transportAttempts: 0,
        transportErrors: [errorCode(error)],
        targetPeerBound: false,
        expectedPeerId,
