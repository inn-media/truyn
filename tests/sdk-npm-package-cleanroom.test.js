...
test('npm alpha.3 externalizes ws as an explicit runtime dependency', () => {
  ...
  assert.equal(version.typescript, '0.1.0-alpha.3');
...
test('ordinary CI clean-room installs and imports the packed TypeScript artifact', () => {
  ...
  assert.match(ci, /truyn-sdk-0\.1\.0-alpha\.3\.tgz/);
