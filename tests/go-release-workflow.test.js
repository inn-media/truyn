...
test('alpha.2 release.5 publishes only through the configured npm Trusted Publisher workflow', () => {
  ...
  assert.match(publishStep, /env -u NODE_AUTH_TOKEN -u NPM_TOKEN -u NPM_CONFIG_TOKEN/);
  assert.match(publishStep, /npm publish[\s\S]*--access public --tag "\$NPM_DIST_TAG" --provenance/);
  assert.doesNotMatch(publishStep, /secrets\./);
  assert.doesNotMatch(supersededWorkflow, /npm publish/);
});

test('Trusted Publishing boundary is token-free and preserves GitHub OIDC', () => {
  const setupStep = workflow.match(/      - name: Set up Node\.js for npm Trusted Publishing\n[\s\S]*?(?=\n      - name: )/)?.[0];
  const boundaryStep = workflow.match(/      - name: Require token-free npm Trusted Publishing boundary\n[\s\S]*?(?=\n      - name: )/)?.[0];
  assert.ok(setupStep && boundaryStep);
  assert.doesNotMatch(setupStep, /registry-url:/);
  assert.match(boundaryStep, /ACTIONS_ID_TOKEN_REQUEST_URL/);
  assert.match(boundaryStep, /ACTIONS_ID_TOKEN_REQUEST_TOKEN/);
  assert.match(boundaryStep, /test -z "\$\{NODE_AUTH_TOKEN:-\}"/);
  assert.match(boundaryStep, /! grep -q '_authToken' "\$npmrc"/);
});
...
test('registry repair marker is exact, immutable and workflow-run tag-bound', () => {
  assert.deepEqual(marker, {
    ...
    npmDistTag: 'alpha',
    npmReleaseTag: 'sdk/npm/v0.1.0-alpha.2-release.5',
    npmSupersedes: '0.1.0-alpha.1',
    npmRepairReason: 'release.3 and release.4 are immutable failed attempts; release.5 isolates npm Trusted Publishing from setup-node token placeholders while preserving exact registry, provenance and PyPI PEP 740 evidence',
...
test('PyPI verifier compares only distribution files and preserves PEP 740 evidence', () => {
  ...
  assert.match(workflow, /TRUYN_SDK_STABLE_API_VERSION/);
  assert.match(workflow, /pep740Provenance/);
