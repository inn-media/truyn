...
test('D-1000 provisioner makes remote install failures actionable and reduces fragile source setup', async () => {
  ...
  assert.match(provision, /WorkingDirectory=\/opt\/truyn\/app/);
  assert.match(provision, /ExecStart=\/opt\/truyn\/runtime\/bin\/node \/opt\/truyn\/app\/network\/testnet\/node-service\.js/);
