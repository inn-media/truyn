...
test('OIDC and attestation permissions are job-scoped away from PR-controlled validation', async () => {
  const workflow = await workflowText();
  const jobsIndex = workflow.indexOf('jobs:');
  const global = workflow.slice(0, jobsIndex);
  assert.doesNotMatch(global, /id-token:\s*write/);
  assert.doesNotMatch(global, /attestations:\s*write/);
  ...
  assert.doesNotMatch(validate, /id-token:\s*write|attestations:\s*write|azure\/login@v2|az acr build/);
