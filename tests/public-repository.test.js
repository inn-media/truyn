...
const ALLOWED_WORKFLOWS = new Set([
  ...
  '.github/workflows/d500-blockwise-launch.yml',
  '.github/workflows/d500-swarm-launch.yml',
...
test('published benchmark evidence is preserved and not replaced by stubs', async () => {
  for (const evidence of protectedBenchmarkEvidence) {
    ...
    catch (error) { assert.fail(`${evidence.path}: protected benchmark evidence is missing (${error.code ?? error.message})`); }
    ...
    for (const marker of evidence.markers) assert.ok(content.includes(marker), `${evidence.path}: protected benchmark evidence lost required marker: ${marker}`);
  }
});

test('temporary D-1000 launcher workflows are never committed to the public workflow tree', async () => {
  const files = await collect();
  const temporaryLaunchers = files.map((f) => f.relative).filter((r) => r.startsWith('.github/workflows/tmp-class-d1000-')).sort();
  assert.deepEqual(temporaryLaunchers, [], `Temporary D-1000 launchers must remain execution scaffolding only:\n${temporaryLaunchers.join('\n')}`);
});

test('D-200 acceptance launcher is frozen to the qualified exact-main evidence', async () => {
  const content = await readFile(path.join(ROOT, '.github/workflows/d200-acceptance.yml'), 'utf8');
  for (const marker of ['TESTED_COMMIT: e91c165c67c655deb80df4511ca346acb9f1f45b','TESTED_TREE_SHA: 3a402ba72502de12ed2277db3c9f472872f44b46',"EXACT_MAIN_CI_RUN: '35500410155'","EXACT_MAIN_FIVE_PATCH_RUN: '35500410293'","QUALIFIED_TREE_CODEQL_CHECK: '106050818849'",'QUALIFIED_TREE_CODEQL_SHA: 293e3cf5da54e58e37a8d3bb3c138b05bf03e36b','truyn-d200-e91c165c-qualified-single-shot']) assert.ok(content.includes(marker), `D-200 launcher lost qualified freeze marker: ${marker}`);
  assert.ok(content.includes('source scripts/d200-stage-isolated-campaign.sh'), 'D-200 launcher must execute the stage-isolated campaign orchestrator');
...
test('public repository contains no known operational/cloud leakage or credential patterns', async () => {
  ...
  for (const file of files) {
    ...
    if (file.relative.startsWith('.github/workflows/') && !ALLOWED_WORKFLOWS.has(file.relative)) violations.push(`${file.relative}: workflow is not on the public allowlist`);
    for (const fragment of forbiddenPathFragments) if (file.relative.includes(fragment)) violations.push(`${file.relative}: forbidden operational path category`);
    for (const pattern of forbiddenPathPatterns) if (pattern.test(file.relative)) violations.push(`${file.relative}: forbidden operational path pattern`);
    ...
    for (const marker of forbiddenLiteralMarkers) {
      ...
      if (STATIC_AZURE_OIDC_WORKFLOWS.has(file.relative) && (marker === 'AZURE_TENANT_ID' || marker === 'AZURE_SUBSCRIPTION_ID')) {
        const exact = '${{ secrets.' + marker + ' }}';
        const remainder = content.split(exact).join('');
        if (remainder.includes(marker)) violations.push(`${file.relative}: forbidden operational marker category outside exact static secret reference`);
        continue;
      }
      if (content.includes(marker)) violations.push(`${file.relative}: forbidden operational marker category`);
    }
    for (const pattern of forbiddenCredentialPatterns) if (pattern.test(content)) violations.push(`${file.relative}: credential/private-key pattern detected`);
    for (const pattern of forbiddenTopologyPatterns) if (pattern.test(content)) violations.push(`${file.relative}: live operational topology pattern detected`);
    if ((file.relative.startsWith('benchmarks/') || file.relative.startsWith('scripts/')) && EXECUTABLE_EXTENSIONS.has(ext)) {
      for (const pattern of forbiddenOperationalExecutablePatterns) if (pattern.test(content)) violations.push(`${file.relative}: operational cloud credential/proxy code detected`);
    }
  }
