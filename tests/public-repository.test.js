...
test('D-200 acceptance launcher is frozen to the qualified exact-main evidence', async()=>{const content=await readFile(path.join(ROOT,'.github/workflows/d200-acceptance.yml'),'utf8');for(const marker of ['TESTED_COMMIT: 1c2a432a447b4b6da5aa952c0c51902ecaae454a','TESTED_TREE_SHA: 290c8d0dc2a80dd787feee2f572462d8d97ce351',"EXACT_MAIN_CI_RUN: '35469639703'","EXACT_MAIN_CODEQL_RUN: '35469639095'",'truyn-d200-1c2a432a-qualified-single-shot'])assert.ok(content.includes(marker),`D-200 launcher lost qualified freeze marker: ${marker}`);assert.ok(content.includes('source scripts/d200-stage-isolated-campaign.sh'),'D-200 launcher must execute the stage-isolated campaign orchestrator')});
