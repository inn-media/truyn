...
test('D-200 recovery repair bounds end-to-end restart budget without weakening acceptance', async () => {
  ...
  const recoveryRpcAttempts = retryDelaysMs.length + 1;
  const deterministicEnvelopeMs = stopTimeoutMs + disruptionPauseMs + retryDelaysMs.reduce((a, b) => a + b, 0) + recoveryRpcAttempts * rpcTimeoutMs;
