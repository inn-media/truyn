...
function sendEventStream(res, events, requestCorrelationId) {
  const data = events.map((event) => `event: ${event.kind.toLowerCase()}\nid: ${event.id}\ndata: ${JSON.stringify(event)}\n\n`).join('');
  res.writeHead(200, {
...
export function createHttpAdapterServer({ node, maxBodyBytes = 256 * 1024, descriptorEnv = process.env }) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      const needEventsRoute = url.pathname.match(/^\/v1\/needs\/([^/]+)\/events$/);
      if (req.method === 'GET' && needEventsRoute) {
        await ensureRegistered();
        const requestId = decodeURIComponent(needEventsRoute[1]);
        const status = await node.requestStatus(requestId);
        const events = [];
        if (status.result?.envelope) {
          events.push({
            id: `${requestId}:result`,
            kind: 'RESULT',
            requestId,
            envelope: status.result.envelope,
            trust: status.result.trust || null
          });
        }
        return sendEventStream(res, events, requestCorrelationId);
      }
      ...
      if (req.method === 'DELETE' && needStatusRoute) {
        await ensureRegistered();
        const requestId = decodeURIComponent(needStatusRoute[1]);
        const before = await node.requestStatus(requestId);
        ...
        const cancelled = await node.revoke(requestId, 'rest_cancelled_by_requester', { targetKind: 'need' });
        return sendJson(res, 200, {
          ok: true,
          requestId,
          cancelled: true,
          idempotent: Boolean(cancelled.idempotent),
