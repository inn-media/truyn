...
export function createA2aServer({
  ...
  async function drainTruynEvents() {
    const run = pollTail.then(async () => {
      ...
      for (const event of polled.events || []) store.completeFromTruynEvent(event);
      return polled;
    });
    pollTail = run.catch(() => {});
    return run;
  ...
  async function sendMessage(params, req) {
    ...
    const policy = {
      ...structuredClone(policyFromHook),
      ...resultMetadataPolicy(task, message, skill)
    };
  ...
  async function getTask(params, req) {
    ...
    if (params.historyLength !== undefined && (!Number.isInteger(params.historyLength) || params.historyLength < 0)) {
      throw a2aError(-32602, 'Invalid historyLength', 'INVALID_PARAMS');
    }
    const principal = await authenticateRequest(req);
    const ownerKey = stablePrincipalKey(principal, principalKey);
    await drainTruynEvents();
  ...
  async function dispatchRpc(body, req) {
    if (!isObject(body) || body.jsonrpc !== '2.0' || typeof body.method !== 'string' || body.method.length === 0) {
      throw a2aError(-32600, 'Invalid Request', 'INVALID_REQUEST');
    }
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/.well-known/agent-card.json') {
        return sendJson(res, 200, await agentCard(null, req, { extended: false }), { vary: 'A2A-Version, Authorization' });
      }
