...
**Status:** current reference operations baseline for `0.1.0-dev`; not a mainnet SRE/SLO acceptance claim. The current production relay origin perimeter is deployment-proven, the numerical production SLI/SLO contract is defined, and standard production observability plus service-alert/error-budget policy are implemented; deployed backends/probes/alert delivery/on-call and durable SLO compliance evidence remain pre-stable.
...
- [Production Alerting and Error Budgets](ALERTING.md) — 28-day budget recording, multi-window SLO burn paging and service/security anomaly alerts; implementation exists, real pager/on-call delivery evidence remains open.
...
The alerting layer now records rolling 28-day error-budget consumption and implements multi-window service pages for availability, 5xx, dispatch, RESULT delivery/timeouts, provider failures and DHT degradation, plus anomaly/zero-budget signals for WebSocket disconnect storms, authorization denies, billing ambiguity, artifact-store failures and origin-bypass probes.

These are still targets and implementation surfaces until real deployment telemetry, independent probes, alert delivery, on-call ownership and durable acceptance evidence satisfy the SLO contract.
...
- test-fired burn-rate/zero-budget alert delivery, on-call ownership and accepted 28-day compliance evidence;
...
A defined SLO target, instrumented dashboard or checked-in alert rule is also not proof of SLO compliance. Productionized status requires real serving-path measurements, auditable exclusions, error-budget accounting, test-fired alert delivery, responsible on-call ownership and durable acceptance evidence.

## Public/private boundary

Public runbooks may document generic configuration names, metric names, failure modes and acceptance invariants. Exact live origins, provider/node IDs, cloud identities, privileged bootstrap sets, secret values, collector credentials, billing accounts, pager destinations and incident-sensitive data remain outside the public repository.
